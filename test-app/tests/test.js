...
test('SVG as inline component', async ({ page }) => {
  ...
  expect(await h2s.nth(0).textContent()).toBe('As inline component')
  expect(await outerHtml(svgs.nth(0))).toEqual(svg0)
})
...
test('SVG as inline string', async ({ page }) => {
  ...
  expect(await h2s.nth(1).textContent()).toBe('As inline string')
  expect(await outerHtml(svgs.nth(1))).toEqual(svg1)
})
...
test('As URL in an image tag', async ({ page }) => {
  ...
  expect(await h2s.nth(2).textContent()).toBe('As URL in an image tag')
...
test('With component props', async ({ page }) => {
  await page.goto('/')
  const h2s = page.locator('h2')
  const svg = page.locator('svg').nth(3)

  expect(await h2s.nth(4).textContent()).toBe('With component props')
...
test('From another directory', async ({ page }) => {
  ...
  expect(await h2s.nth(5).textContent()).toBe('From another directory')
  expect(await outerHtml(svgs.nth(4))).toEqual(svg4)
})
